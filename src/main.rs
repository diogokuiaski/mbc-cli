...
use api::client::MercadoBitcoinClient;
use config::Config;
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::OrderBook { coin } => {
            ...
            let orderbook = client.get_orderbook(&coin).await?;
...
async fn analyze_with_strategy(
    ...
    for i in 0..num_ticks {
        let ticker = client.get_ticker(coin).await?;
        let price = ticker.ticker.last.parse::<f64>().unwrap_or(0.0);
        
        let signal = strategy.analyze(price);
        signals.push(signal);
